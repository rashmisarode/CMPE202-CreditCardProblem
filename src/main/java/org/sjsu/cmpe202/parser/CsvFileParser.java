            ...
            while ((line = reader.readNext()) != null) {
                ...
                if(count>0) {
                    Record record1 = RecordBuilder.aRecord()
                            ...
                            .withCcNumber(line[0])
                            .withExpDate(line[1])
                            .withCcHolderName(line[2])
                            .build();
                   // System.out.println("Credit Card [id= " + record1.getRecordID() + ",cardNumber= " + record1.getCcNumberStr() + ", code= " + record1.getExpDate() + " , name=" + record1.getCcHolderName() + "]");
                    result.add(record1);
                }
                count++;
            }
    ...
    public void write(List<Record> records, String outputFileName) throws IOException {
        ...
        try {
            ...
            for(int i =0; i< records.size(); i++){
                csvWriter.writeNext(new String[]{records.get(i).getCcNumberStr(), String.valueOf(records.get(i).getCcType()), records.get(i).getError()});
            }
            csvWriter.flush();
        } catch (IOException e) {
            e.printStackTrace();
